...
from util.log_util import logger4movie_based_recall as logger
...
# 处理主函数
def func_main():
    logger.info('start process movie based recall task:'+str(datetime.datetime.now()))
    ...
    for i in df_comment_new_data_ldim.index:
        logger.info((df_comment_new_data_ldim.iloc[i]['MOVIEID'], df_comment_new_data_ldim.iloc[i]['USERID']))
        # 从相似推荐中找到相似的电影
    ...
    logger.info('finish process movie based recall task:' + str(datetime.datetime.now()))
